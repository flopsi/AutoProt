...
# --- Thermo Fisher Brand Colors (Light + Dark variants) ---
COLORS = {
    "red": "#E71316",
    "dark_red": "#A6192E",
    "gray": "#54585A",
...
with st.sidebar:
    st.markdown("""
    <div class="sidebar-header" style="padding: 15px; margin: -60px -15px 20px -15px;">
    ...
    if st.session_state.peptide_data is not None:
        ...
        st.success(f"✓ Peptide data: {n_rows:,} rows")
    else:
        st.caption("No peptide data cached")
    
    st.markdown("---")
    st.caption("© 2024 Thermo Fisher Scientific Inc.")
...
if uploaded_file:
    ...
    if species_tags and filter_col:
        ...
        st.info(f"ℹ Filtered to {len(processed_df):,} rows matching: {', '.join(species_tags)}")
    ...
    st.caption(f"Missing values in numeric columns will be replaced with 1. Protein group column (`{protein_group_col}`) will be preserved for mapping.")
    
    col_btn1, col_btn2, _ = st.columns([1, 1, 3])
    with col_btn1:
        if st.button("✓ Confirm & cache", type="primary"):
            cache_df = processed_df.copy()
            cache_df[numeric_final] = cache_df[numeric_final].fillna(1)
...
if st.session_state.protein_data is not None or st.session_state.peptide_data is not None:
    ...
    with tab1:
        if st.session_state.protein_data is not None:
            idx_col = st.session_state.protein_index_col
            st.caption(f"Index column for mapping: `{idx_col}`")
    ...
    with tab2:
        if st.session_state.peptide_data is not None:
            idx_col = st.session_state.peptide_index_col
            st.caption(f"Index column for mapping: `{idx_col}`")
