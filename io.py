...
File loading, column detection, validation, and export functions.
All functions are production-grade with error handling and type hints.
"""
...
def load_excel(file_path: str, sheet_name: int = 0, **kwargs) -> pd.DataFrame:
    ...
    Args:
        ...
        sheet_name: Sheet number to load (0-indexed)
...
def detect_numeric_columns(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    ...
    Detects columns that are already numeric OR can be converted to numeric
    (e.g., string-formatted numbers like "682.63").
    ...
    Returns:
        ...
        - numeric_cols: Columns that are numeric or convertible to numeric
    ...
    for col in df.columns:
        ...
        if df[col].dtype == 'object' or df[col].dtype == 'string':
            try:
                # Attempt conversion to numeric (errors='coerce' converts non-numeric to NaN)
                converted = pd.to_numeric(df[col], errors='coerce')
                
                # Check if MOST values converted successfully (allow up to 5% NaN from conversion)
...
def convert_string_numbers_to_float(df: pd.DataFrame, numeric_cols: List[str]) -> pd.DataFrame:
    ...
    Should be called AFTER detect_numeric_columns() identifies the columns.
...
def detect_sample_columns(columns: List[str]) -> List[str]:
    ...
    Uses heuristic: numeric-sounding names, not ID-like.
    ...
    """
    id_keywords = [
        'id', 'name', 'identifier', 'gene', 'protein', 'uniprot',
