...
Functions to clean, trim, and standardize column names for display and analysis.
"""
...
def trim_name(name: str, max_length: int = 20) -> str:
    ...
    Example:
        >>> trim_name("Sample_Condition_A_Rep1", max_length=15)
        "Sample_Conditi…"
    """
    name = str(name).strip()
...
def abbreviate_name(name: str, style: str = 'short') -> str:
    ...
    Example:
        >>> abbreviate_name("Sample_Condition_A", style='smart')
        "SCA"
    """
    name = str(name).strip()
...
def standardize_condition_names(columns: List[str]) -> Dict[str, str]:
    ...
    Example:
        >>> standardize_condition_names(['A1', 'A2', 'B1', 'B2'])
        {'A1': 'A_R1', 'A2': 'A_R2', 'B1': 'B_R1', 'B2': 'B_R2'}
    """
    mapping = {}
    ...
    for col in columns:
        ...
        if match_end:
            replicate = match_end.group(1).lstrip('0') or '0'
...
def rename_columns_for_display(
    ...
    Example:
        >>> df = pd.DataFrame({'Sample_Condition_A_R1': [1, 2]})
        >>> renamed, mapping = rename_columns_for_display(df, ['Sample_Condition_A_R1'], style='smart')
        >>> mapping
        {'Sample_Condition_A_R1': 'SCR'}
    """
    if style == 'trim':
...
def reverse_name_mapping(mapping: Dict[str, str]) -> Dict[str, str]:
    ...
    Returns:
        Reversed mapping dictionary
    
    Example:
        >>> original = {'A1': 'A_R1', 'A2': 'A_R2'}
        >>> reverse_name_mapping(original)
        {'A_R1': 'A1', 'A_R2': 'A2'}
    """
...
def create_label_rotation_angle(
    ...
    """
    avg_length = sum(len(str(col)) for col in columns) / len(columns) if columns else 0
...
def is_name_too_long(name: str, threshold: int = 20) -> bool:
    ...
    Args:
        name: Column name
        threshold: Maximum allowed length
    
    Returns:
        True if name exceeds threshold
    """
    return len(str(name)) > threshold


def validate_names(columns: List[str]) -> Dict[str, list]:
    """
    ...
    Example:
        >>> validate_names(['A1', 'Sample_Condition_A_Rep1', 'Test@123'])
        {'valid': ['A1'], 'too_long': ['Sample_Condition_A_Rep1'], 'has_special': ['Test@123']}
    """
    valid = []
    too_long = []
    has_special = []
