...
def evaluate_transformation_metrics(
    ...
    """Compute Shapiro p and mean–variance correlations for raw vs transformed."""
    ...
    # limit Shapiro to 5000 points for speed/validity
    ...
    corr_raw = safe_corr(means_raw.to_numpy(), vars_raw.to_numpy())
    corr_trans = safe_corr(means_trans.to_numpy(), vars_trans.to_numpy())
...
def create_evaluation_figure(
    ...
    """Create 2×3 grid of plots as in the Colab screenshot for a single transform."""
    fig = make_subplots(
        rows=2,
        cols=3,
        subplot_titles=[
    ...
    # -------- 3. Q-Q raw --------
    if len(raw_vals) >= 10:
        osm, osr = stats.probplot(raw_vals, dist="norm", fit=True)[:2]
        theo_q = osm[0]
        ordered_vals = osm[1]
        fig.add_trace(
            go.Scatter(
    ...
    if len(trans_vals) >= 10:
        osm_t, osr_t = stats.probplot(trans_vals, dist="norm", fit=True)[:2]
        theo_q_t = osm_t[0]
        ord_t = osm_t[1]
        fig.add_trace(
            go.Scatter(
    ...
    fig.update_xaxes(type="log", row=2, col=2, title_text="Mean")
    fig.update_yaxes(type="log", row=2, col=2, title_text="Variance")
    ...
    fig.update_xaxes(type="linear", row=2, col=3, title_text="Mean")
    fig.update_yaxes(type="linear", row=2, col=3, title_text="Variance")
