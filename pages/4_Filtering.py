...
# After the F calculation, add:
print(f"ss_total: {ss_total:.2f}, ss_between: {ss_between:.2f}, ss_within: {ss_within:.2f}")
print(f"F-stat: {F:.2f}, df_between: {df_between}, df_within: {df_within}")
st.markdown("## Protein-level Filtering & QC")
...
# Species-specific optimization workflow (simplified for brevity)
if use_species_optimization:
    st.info("📋 **Sequential Selection**: Configure and add each species separately. Coming soon in this implementation.")
    # Implementation from previous version can be added here
...
# SD range slider - ALWAYS VISIBLE
st.markdown("#### Standard Deviation Range (drag slider to adjust)")
sd_range = st.slider(
    "Filter range (standard deviations from mean)",
    min_value=0.5,
...
if use_sd_filter:
    ...
    st.info("ℹ️ SD filter disabled. Slider shows reference bounds only.")
