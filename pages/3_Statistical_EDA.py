...
# ----------------------------------------------------------------------
# 4) Quality Filtering Metrics
# ----------------------------------------------------------------------
st.subheader("4️⃣ Quality Filtering Metrics")

st.markdown(
    """
    Use these metrics to identify and filter low-quality proteins before differential analysis.
    """
)

# Compute quality metrics per protein
def compute_quality_metrics(row, cond_a_cols, cond_b_cols):
    ...
    # 5. Reproducibility: correlation between replicates within condition
    if len(vals_a) >= 2:
        corr_a = vals_a.corr(method='pearson').mean() if vals_a.notna().sum() >= 2 else np.nan
    else:
        corr_a = np.nan
    
    if len(vals_b) >= 2:
        corr_b = vals_b.corr(method='pearson').mean() if vals_b.notna().sum() >= 2 else np.nan
    else:
        corr_b = np.nan
...
# Apply metrics
quality_metrics = df.apply(
    lambda row: pd.Series(compute_quality_metrics(row, cond_a_cols, cond_b_cols)),
    axis=1
)

df_quality = pd.concat([df[[species_col]], quality_metrics], axis=1)
...
with col_f1:
    max_missing = st.slider(
        ...
        value=50,
        step=5,
        help="Remove proteins with >X% missing values across samples"
    ) / 100
    ...
    min_detection = st.slider(
        ...
        value=50,
        step=5,
        help="Keep proteins detected in >X% of samples"
    ) / 100

with col_f2:
    min_intensity_percentile = st.slider(
        "Min Intensity Percentile",
        min_value=0,
        max_value=50,
        value=10,
        step=5,
        help="Remove bottom X% of proteins by mean intensity"
    )
    
    min_intensity_threshold = np.percentile(
        df_quality['mean_intensity'].dropna(), 
        min_intensity_percentile
    )

with col_f3:
    filter_by_cv = st.checkbox(
        "Filter by CV",
        value=False,
        help="Remove proteins with CV > threshold"
    )
...
# Apply filters
mask_missing = df_quality['missing_rate'] <= max_missing
mask_detection = df_quality['detection_rate'] >= min_detection
mask_intensity = df_quality['mean_intensity'] >= min_intensity_threshold

if filter_by_cv:
    mask_cv_a = df['CV_CondA'] <= max_cv
    mask_cv_b = df['CV_CondB'] <= max_cv
    mask_cv = mask_cv_a & mask_cv_b
else:
    mask_cv = pd.Series([True] * len(df), index=df.index)

# Combined filter mask
filter_mask = mask_missing & mask_detection & mask_intensity & mask_cv
...
with col_r2:
    st.metric(
        "Filtered Proteins",
        f"{len(df_filtered):,}",
        delta=f"-{len(df) - len(df_filtered):,}",
        delta_color="inverse"
    )
...
st.markdown("#### Retention by Species")

species_summary = []
for species in ["HUMAN", "YEAST", "ECOLI"]:
    original = (df[species_col] == species).sum()
    filtered = (df_filtered[species_col] == species).sum()
    species_summary.append({
        "Species": species,
        "Original": original,
        "Retained": filtered,
        "Retention %": (filtered / original * 100) if original > 0 else 0,
    })

species_summary_df = pd.DataFrame(species_summary)
st.dataframe(species_summary_df, use_container_width=True)

# Visualization: before/after distributions
st.markdown("### Quality Metrics: Before vs After Filtering")

fig_quality = make_subplots(
    rows=2, cols=2,
    subplot_titles=(
        "Detection Rate", "Mean Intensity", 
        "Missing Rate", "CV Distribution"
    ),
)

# Detection rate
fig_quality.add_trace(
    go.Histogram(x=df_quality['detection_rate'], name='Before', 
                 marker_color='lightgray', opacity=0.7, nbinsx=30),
    row=1, col=1
)
fig_quality.add_trace(
    go.Histogram(x=df_quality[filter_mask]['detection_rate'], name='After', 
...
# Mean intensity
fig_quality.add_trace(
    go.Histogram(x=df_quality['mean_intensity'], name='Before', 
                 marker_color='lightgray', opacity=0.7, nbinsx=30, showlegend=False),
    row=1, col=2
)
fig_quality.add_trace(
    go.Histogram(x=df_quality[filter_mask]['mean_intensity'], name='After', 
                 marker_color='#1f77b4', opacity=0.7, nbinsx=30, showlegend=False),
    row=1, col=2
)

# Missing rate
fig_quality.add_trace(
    go.Histogram(x=df_quality['missing_rate'], name='Before', 
                 marker_color='lightgray', opacity=0.7, nbinsx=30, showlegend=False),
    row=2, col=1
)
fig_quality.add_trace(
    go.Histogram(x=df_quality[filter_mask]['missing_rate'], name='After', 
                 marker_color='#1f77b4', opacity=0.7, nbinsx=30, showlegend=False),
    row=2, col=1
)

# CV distribution
cv_all = pd.concat([df['CV_CondA'], df['CV_CondB']]).dropna()
cv_filt = pd.concat([df_filtered['CV_CondA'], df_filtered['CV_CondB']]).dropna()

fig_quality.add_trace(
    go.Histogram(x=cv_all, name='Before', 
                 marker_color='lightgray', opacity=0.7, nbinsx=30, showlegend=False),
    row=2, col=2
)
fig_quality.add_trace(
    go.Histogram(x=cv_filt, name='After', 
                 marker_color='#1f77b4', opacity=0.7, nbinsx=30, showlegend=False),
    row=2, col=2
)

fig_quality.update_xaxes(title_text="Detection Rate", row=1, col=1)
fig_quality.update_xaxes(title_text="Mean Intensity", row=1, col=2)
fig_quality.update_xaxes(title_text="Missing Rate", row=2, col=1)
fig_quality.update_xaxes(title_text="CV (%)", row=2, col=2)
...
if st.button("✅ Apply Filters and Continue", type="primary", use_container_width=True):
    protein_data.raw = df_filtered
    st.session_state.protein_data = protein_data
    st.success(f"✅ Filters applied! {len(df_filtered):,} proteins retained.")
    st.rerun()

st.info("💡 Adjust thresholds above, review results, then click 'Apply Filters' to continue with filtered data.")
