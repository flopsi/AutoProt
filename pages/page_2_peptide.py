...
# ========================================================================
st.set_page_config(
    page_title="Peptide Upload | Proteomics",
    page_icon="🧪",
    layout="wide"
)

# Apply brand/theme CSS same as on Protein page
st.markdown(f"""
<style>
:root {{
...
# HEADER
# ========================================================================
st.title("🧪 Peptide-Level Data Upload")
st.caption("Import and configure peptide-level quantification data")
...
<div class="step-header">
  ...
  <span>Upload Peptide-Level File</span>
</div>
""", unsafe_allow_html=True)

uploaded_file = st.file_uploader(
    "Choose peptide quantification file",
    type=['csv', 'tsv', 'txt'],
    key="peptide_file_upload",
    help="Supported formats: CSV, TSV, TXT"
)

if not uploaded_file:
    st.info("📁 Please upload a peptide-level quantification file to begin, or skip if not needed.")
    st.stop()
...
default_peptide_cols = get_default_peptide_id_col(preview_df, meta_cols)
...
col_order = ['Include', 'Trimmed Name', 'Protein Group', 'Species Mapping', 'Peptide', 'Control', 'Treatment', 'Original Name']
    ...
    with st.spinner("Loading full dataset..."):
        ...
        for _, row in included_rows.iterrows():
            ...
            col_meta = ColumnMetadata(
                ...
                is_peptide_id=row['Peptide'],
                condition=cond
            )
            columns_metadata.append(col_meta)

        config = DatasetConfig(
            level=DataLevel.PEPTIDE,
            ...
            peptide_id_col=rename_map.get(peptide_cols[0]) if peptide_cols else None
        )
        dataset = ProteomicsDataset(config=config, data=full_df)

        # Store in session_state
        st.session_state[SessionKeys.PEPTIDE_DATASET.value] = dataset

        st.success(f"✅ **Dataset loaded successfully!** {len(full_df):,} peptide rows")
        st.balloons()
...
if SessionKeys.PEPTIDE_DATASET.value in st.session_state:
    ...
    st.write(f"🔹 **Peptide ID:** {dataset.config.peptide_id_col}")
    ...
    st.info("✨ **Next Step:** Go to the Analysis page to begin your statistical workflow.")
