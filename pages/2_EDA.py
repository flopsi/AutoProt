...
def create_missing_distribution_chart(df: pd.DataFrame, numeric_cols: list[str], 
                                       label: str = "protein groups") -> go.Figure:
    # Count missing values (value == 1) per row
    missing_per_row = (df[numeric_cols] == 1).sum(axis=1)
    total_rows = len(df)

    # Count how many rows have 0, 1, 2, ... n missing values
...
with tab_protein:
    if protein_data is not None:
        ...
        with col1:
            fig_heat = create_intensity_heatmap(protein_data, protein_idx, numeric_cols)
            st.plotly_chart(fig_heat, use_container_width=True)

        with col2:
            fig_bar = create_missing_distribution_chart(protein_data, numeric_cols, "protein groups")
...
with tab_peptide:
    if peptide_data is not None:
        ...
        with col1:
            fig_heat = create_intensity_heatmap(peptide_data, peptide_idx, numeric_cols)
            st.plotly_chart(fig_heat, use_container_width=True)

        with col2:
            fig_bar = create_missing_distribution_chart(peptide_data, numeric_cols, "peptides")
            st.plotly_chart(fig_bar, use_container_width=True)
    else:
