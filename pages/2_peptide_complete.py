...
if "pep_df" in st.session_state:
    ...
    seq_col = st.session_state.pep_seq_col
    ...
    st.info(f"**Species** → `{sp_col}` | **Sequence** → `{seq_col}`")
    
    if sp_counts is not None and len(sp_counts) > 0:
        st.markdown("### 📊 Peptides by Species & Condition")
    ...
    if st.button("🔄 Restart Analysis", use_container_width=True, key="restart_pep"):
...
with st.container():
    ...
    with col1:
        uploaded_file = st.file_uploader("", type=["csv","tsv","txt"], label_visibility="collapsed", key="pep_upload")
    with col2:
        skip_upload = st.button("⏭️ Skip", key="skip_pep_btn")

if skip_upload:
    st.session_state["skip_pep"] = True
    st.info("⏭️ Peptide upload skipped")
    st.markdown('<div class="footer"><strong>Proprietary & Confidential</strong><br>© 2024 Thermo Fisher Scientific</div>', unsafe_allow_html=True)
    st.stop()
...
def load_and_parse(file):
    ...
    intensity_cols = [c for c in df.columns if c not in ["pg", "name", "Protein.Group", "PG.ProteinNames", "PEP.StrippedSequence", "Sequence", "Accession", "Species"]]
...
st.success(f"✅ Data imported — {len(df):,} peptides")
...
with st.container():
    ...
    meta_cols_list = ["Protein.Group", "PG.ProteinNames", "PEP.StrippedSequence", "Sequence", "Accession", "Species"]
    ...
    seq_col_candidates = ["PEP.StrippedSequence", "Stripped.Sequence", "Sequence", "peptide"]
    seq_col_default = next((c for c in seq_col_candidates if c in df.columns), df.columns[0])
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "Sequence": st.column_config.CheckboxColumn("Seq", default=True),
    ...
    seq_checked = edited[edited["Sequence"]]
    ...
    if len(seq_cols) != 1:
        errors.append("❌ Select exactly 1 Sequence column")
...
st.session_state.update({
    ...
    "pep_seq_col": seq_col,
    "pep_sp_ext": sp_ext,
    "pep_species": species_list,
    "pep_sp_counts": sp_counts,
    "pep_n": len(df),
    "pep_rename": rename_map,
    "pep_num_cols": num_cols,
})
...
# ─────────────────────────────────────────────────────────────
st.markdown("""<style>.fixed-restart {position: fixed; bottom: 20px; left: 50%; transform: translateX(-50%); z-index: 999; width: 300px;}</style><div class="fixed-restart">""", unsafe_allow_html=True)
if st.button("🔄 Restart Analysis", use_container_width=True, key="restart_pep_final"):
    st.session_state.clear()
    st.rerun()
st.markdown('</div>', unsafe_allow_html=True)
