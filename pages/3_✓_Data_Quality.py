    ...
    for idx, (col, condition) in enumerate(all_samples[:6]):
        ...
        fig_cv_panel.add_trace(
            go.Bar(
                ...
                marker_color=ThermoFisherColors.PRIMARY_RED if condition[0] == 'A' else '#9BD3DD',
...
with data_tab2:
    if peptide_uploaded:
        ...
        # DUPLICATE ALL PLOTS HERE WITH PEPTIDE DATA
        # (Same code as above, just using peptide_data instead)
        
        condition_mapping = current_data.condition_mapping
        quant_data = current_data.quant_data
        species_map = current_data.species_map
        
        # [All 6 plots repeated here with peptide data]
        st.info("Peptide visualizations - Same plots as protein tab, using peptide data")
